...
def
@contextmanager
def sqlite_s3_query(url, s3_client=lambda: boto.client('s3'),
   get_libsqlite3=lambda: cdll.LoadLibrary({'linux': 'libsqlite3.so.0', 'darwin': 'libsqlite3.dylib'}[platform])):
    ...
    @contextmanager
    def get_vfs(s3_client):
        with s3_client.head_object(
            Bucket=bucket,
            Key=path

        ) as object_info:
        logger.debug(f"{object_info}")

        # this tag can be missing when working with unversioned S3 buckets
        version_id = object_info.get('VersionId') 
        size = int(object_info['ContentLength'])
